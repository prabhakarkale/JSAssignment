...
function checkAddTodoValidation(newtodo){
    ...
    else if((document.getElementById('chkYes').checked == true) && (new Date(newtodo.rdate).getTime() < Todate.getTime() )){
...
function updateTodoData()
{
    ...
    var users = JSON.parse(localStorage.getItem('users'));
    var todo = users[userIndex].todo[todoIndex];
...
{    alert(title)
    document.getElementById('showdata').style.display = "block"
    var text= "";
    text+=`<tr><th><input type="checkbox"></th>`;
